...
describe("As a user i want to update an exercice", () => {
    ...
    describe("Given two exercices are created", () => {
        ...
        describe("When the exercice update has not started", () => {
            ...
            test("Then it should set the loading to true", async () => {
                expect(testStore.getState().exercices.update.isLoading).toBe(false);
            });

            test("Then the list should contains the original exercices", async () => {
                expect(testStore.getState().exercices.list.exercices).toBe(exercices);
            });
    ...
    describe("Given two exercices are created", () => {
        ...
        describe("When the exercice update starts", () => {
            ...
            test("Then it should set the loading to true", async () => {
                expect(testStore.getState().exercices.update.isLoading).toBe(true);
            });
        });
    });
    ...
    describe("Given two exercices are created", () => {
        ...
        describe("When the exercice is updated successfully", () => {
            ...
            test("Then it should set the updated exercice as current exercice", async () => {
                expect(testStore.getState().exercices.list.exercices[0].id).toEqual(exerciceIdToUpdate,);
                expect(testStore.getState().exercices.list.exercices[0].title).toEqual(updateExerciceCommand.title,);
            });

            test("Then it should set a success notification", async () => {
                const updateSuccessNotification = testStore
                    .getState()
    ...
    describe("Given two exercices are created", () => {
        ...
        describe("When the exercice update fails", () => {
            ...
            test("Then it should set an error notification", async () => {
                const updateErrorNotification = testStore
                    .getState()
                    .notifications.list.find((notification) => notification.message === "Exercice maj échouée",);
