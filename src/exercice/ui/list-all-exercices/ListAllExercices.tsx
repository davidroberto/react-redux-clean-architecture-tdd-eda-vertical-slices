...
import {ExercicesSortedByMuscle} from "@/src/exercice/features/shared/exercice.model.type";
...
export default function ListAllExercices() {
    const {exercices, isLoading, navigateToCreateExercice} =
        uselistAllExercices();
    ...
    const renderMuscle = (muscle: ExercicesSortedByMuscle) => (
        <View key={muscle.id} style={styles.muscleContainer}>
            <Text style={styles.muscleTitle}>{muscle.title}</Text>
            {muscle.exercises && muscle.exercises.length > 0 && (
                <FlatList
            ...
            {muscle.children && muscle.children.length > 0 && (
